...
export function StepIngreso({ data, onData, onBack, onNext, solicitantes }: { data: WizardData; onData: (d: WizardData) => void; onBack: () => void; onNext: () => void; solicitantes: SolicitanteIngreso[] }) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field>
              ...
              <Select
                ...
                onValueChange={(v) => set({ id_solicitante_ingreso: v === "none" ? "" : v })}
              >
            ...
            <Field>
              ...
              {detectTipoCausa(ing.codigo_rit) && (
                <p className="text-xs text-muted-foreground mt-1">
                  <Badge variant="secondary">{detectTipoCausa(ing.codigo_rit)}</Badge>
                </p>
              )}
            </Field>
            <Field>
          ...
          <div className="border-t pt-4">
            ...
            {ing.causales.map((c, i) => (
              <div key={i} className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3 p-3 border rounded-lg">
                <Select value={c.nombre_causal || "none"} onValueChange={(v) => {
                  const cs = [...ing.causales]; cs[i] = { ...cs[i], nombre_causal: v === "none" ? "" : v }; set({ causales: cs });
                }}>
          ...
          <div className="border-t pt-4">
            ...
            {ing.derechos.map((d, i) => (
              <div key={i} className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3 p-3 border rounded-lg">
                <Select value={d.nombre_derecho || "none"} onValueChange={(v) => {
                  const ds = [...ing.derechos]; ds[i] = { ...ds[i], nombre_derecho: v === "none" ? "" : v }; set({ derechos: ds });
                }}>
                  <SelectTrigger><SelectValue placeholder="Seleccionar derecho" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">Ninguno</SelectItem>
