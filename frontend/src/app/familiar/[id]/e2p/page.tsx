...
export default function E2PPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updated = await api.e2p.update(editingItem.id_e2p, payload);
      setItems((prev) => prev.map((i) => (i.id_e2p === editingItem.id_e2p ? updated : i)));
      setDialogOpen(false);
    } catch (e: unknown) {
              ...
              <CardContent className="pt-4">
                <div className="flex items-start justify-between">
                  <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
                    ...
                    <div><span className="text-xs text-muted-foreground">Rango etario: </span>{item.rango_etario || "\u2014"}</div>
                    <div><span className="text-xs text-muted-foreground">Edad (meses): </span>{item.edad_meses_evaluacion ?? "\u2014"}</div>
