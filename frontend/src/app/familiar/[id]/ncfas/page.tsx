            ...
            <Card key={item.id_ncfas}>
              <CardContent className="pt-4">
                <div className="flex items-start justify-between">
                  ...
                  <Button variant="ghost" size="icon" aria-label={`Editar NCFAS${item.estado ? `: ${item.estado}` : ""}`} onClick={() => openEdit(item)}><PencilIcon className="size-4" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            <SelectField label="Estado" value={editEstado} onValueChange={setEditEstado}>
              <SelectGroup>
            ...
            <DateField label="Fecha apertura" value={editFechaApertura} onChange={setEditFechaApertura} />
            <DateField label="Fecha cierre" value={editFechaCierre} onChange={setEditFechaCierre} />
            <TextField label="Observación general" value={editObservacion} onChange={(e) => setEditObservacion(e.target.value)} placeholder="Observaciones" />
