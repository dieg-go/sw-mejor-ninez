...
import { type HistorialRedProteccional } from "@/lib/api";
import {
  useNNA,
  useHistorialRedList,
  useHistorialRedCreate,
  useHistorialRedUpdate,
} from "@/lib/queries";
...
export default function HistorialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: items = [] } = useHistorialRedList(id);
  const createMut = useHistorialRedCreate(id);
  const updateMut = useHistorialRedUpdate();
  ...
  const startEdit = (item: HistorialRedProteccional) => {
    ...
    setEditForm({ nombre_programa: item.nombre_programa || "", fecha_ingreso: item.fecha_ingreso || "", fecha_egreso: item.fecha_egreso || "", motivo_egreso: item.motivo_egreso || "" });
          ...
          <CardContent>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><Label className="text-xs">Nombre programa</Label><Input className="mt-1" value={form.nombre_programa} onChange={(e) => setForm((p) => ({ ...p, nombre_programa: e.target.value }))} placeholder="Programa" /></div>
                <div><Label className="text-xs">Motivo egreso</Label><Input className="mt-1" value={form.motivo_egreso} onChange={(e) => setForm((p) => ({ ...p, motivo_egreso: e.target.value }))} placeholder="Motivo" /></div>
                <div>
                  <Label className="text-xs">Fecha ingreso</Label>
                  <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !fechaIngreso && "text-muted-foreground")}><CalendarIcon />{fechaIngreso ? fechaIngreso.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={fechaIngreso} onSelect={setFechaIngreso} /></PopoverContent></Popover>
                </div>
                <div>
                  <Label className="text-xs">Fecha egreso</Label>
                  <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !fechaEgreso && "text-muted-foreground")}><CalendarIcon />{fechaEgreso ? fechaEgreso.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={fechaEgreso} onSelect={setFechaEgreso} /></PopoverContent></Popover>
                </div>
              </div>
              ...
              <CardContent className="pt-4">
                {editingId === item.id_historial_red ? (
                  <form onSubmit={handleUpdate} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><Label className="text-xs">Nombre programa</Label><Input className="mt-1" value={editForm.nombre_programa} onChange={(e) => setEditForm((p) => ({ ...p, nombre_programa: e.target.value }))} /></div>
                      <div><Label className="text-xs">Motivo egreso</Label><Input className="mt-1" value={editForm.motivo_egreso} onChange={(e) => setEditForm((p) => ({ ...p, motivo_egreso: e.target.value }))} /></div>
                      <div><Label className="text-xs">Fecha ingreso</Label>
                        <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !editFechaIngreso && "text-muted-foreground")}><CalendarIcon />{editFechaIngreso ? editFechaIngreso.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger>
                          <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={editFechaIngreso} onSelect={setEditFechaIngreso} /></PopoverContent></Popover>
                      </div>
                      <div><Label className="text-xs">Fecha egreso</Label>
                        <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !editFechaEgreso && "text-muted-foreground")}><CalendarIcon />{editFechaEgreso ? editFechaEgreso.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger>
                          <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={editFechaEgreso} onSelect={setEditFechaEgreso} /></PopoverContent></Popover>
                      </div>
                    </div>
