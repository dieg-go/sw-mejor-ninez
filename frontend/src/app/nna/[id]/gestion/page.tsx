...
import { type GestionBusquedaFamiliar } from "@/lib/api";
import {
  useNNA,
  useGestionBusquedaList,
  useGestionBusquedaCreate,
  useGestionBusquedaUpdate,
} from "@/lib/queries";
...
export default function GestionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: items = [] } = useGestionBusquedaList(id);
  const createMut = useGestionBusquedaCreate(id);
  const updateMut = useGestionBusquedaUpdate();
  ...
  const startEdit = (item: GestionBusquedaFamiliar) => {
    ...
    setEditForm({ tipo_gestion: item.tipo_gestion || "", fecha_solicitud_envio: item.fecha_solicitud_envio || "", fecha_respuesta_recepcion: item.fecha_respuesta_recepcion || "", resultado: item.resultado || "", comprobante_adjunto: item.comprobante_adjunto });
          ...
          <CardContent>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><Label className="text-xs">Tipo gestión</Label><Input className="mt-1" value={form.tipo_gestion} onChange={(e) => setForm((p) => ({ ...p, tipo_gestion: e.target.value }))} placeholder="Tipo de gestión" /></div>
                ...
                <div><Label className="text-xs">Fecha solicitud</Label><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !fechaSolicitud && "text-muted-foreground")}><CalendarIcon />{fechaSolicitud ? fechaSolicitud.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={fechaSolicitud} onSelect={setFechaSolicitud} /></PopoverContent></Popover></div>
                <div><Label className="text-xs">Fecha respuesta</Label><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !fechaRespuesta && "text-muted-foreground")}><CalendarIcon />{fechaRespuesta ? fechaRespuesta.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={fechaRespuesta} onSelect={setFechaRespuesta} /></PopoverContent></Popover></div>
                <div className="flex items-center gap-2 pt-2"><Checkbox id="comprobante" checked={form.comprobante_adjunto} onCheckedChange={(v) => setForm((p) => ({ ...p, comprobante_adjunto: !!v }))} /><Label htmlFor="comprobante" className="text-xs cursor-pointer">Comprobante adjunto</Label></div>
              </div>
              ...
              <CardContent className="pt-4">
                {editingId === item.id_gestion_busqueda ? (
                  <form onSubmit={handleUpdate} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><Label className="text-xs">Tipo gestión</Label><Input className="mt-1" value={editForm.tipo_gestion} onChange={(e) => setEditForm((p) => ({ ...p, tipo_gestion: e.target.value }))} /></div>
                      ...
                      <div><Label className="text-xs">Fecha solicitud</Label><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !editFechaSolicitud && "text-muted-foreground")}><CalendarIcon />{editFechaSolicitud ? editFechaSolicitud.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={editFechaSolicitud} onSelect={setEditFechaSolicitud} /></PopoverContent></Popover></div>
                      <div><Label className="text-xs">Fecha respuesta</Label><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal mt-1", !editFechaRespuesta && "text-muted-foreground")}><CalendarIcon />{editFechaRespuesta ? editFechaRespuesta.toLocaleDateString("es-CL") : "Seleccionar"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={editFechaRespuesta} onSelect={setEditFechaRespuesta} /></PopoverContent></Popover></div>
                      <div className="flex items-center gap-2 pt-2"><Checkbox id={`edit-comp-${item.id_gestion_busqueda}`} checked={editForm.comprobante_adjunto} onCheckedChange={(v) => setEditForm((p) => ({ ...p, comprobante_adjunto: !!v }))} /><Label htmlFor={`edit-comp-${item.id_gestion_busqueda}`} className="text-xs cursor-pointer">Comprobante</Label></div>
                    </div>
