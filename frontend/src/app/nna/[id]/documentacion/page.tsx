...
import { DateField, SelectField, TextField } from "@/components/ui/form-field";
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <SelectField
                  ...
                  value={form.tipo_documento || "none"}
                  onValueChange={(v) => setForm((p) => ({ ...p, tipo_documento: v === "none" ? "" : v }))}
                >
                ...
                <DateField label="Fecha recepción" value={fecha} onChange={setFecha} />
                ...
                <TextField label="Observación" value={form.observacion} onChange={(e) => setForm((p) => ({ ...p, observacion: e.target.value }))} placeholder="Observaciones" />
                ...
                {editingId === item.id_documentacion ? (
                  <form onSubmit={handleUpdate} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <SelectField
                        label="Tipo documento"
                        value={editForm.tipo_documento || "none"}
                        onValueChange={(v) => setEditForm((p) => ({ ...p, tipo_documento: v === "none" ? "" : v }))}
                      >
                        <SelectItem value="none">Ninguno</SelectItem>
                        {CATALOGO_DOCUMENTACION_INGRESO.map((tipo) => (
                          <SelectItem key={tipo} value={tipo}>{tipo}</SelectItem>
                      ...
                      <DateField label="Fecha recepción" value={editFecha} onChange={setEditFecha} />
                      <div className="flex items-center gap-2 pt-2">
                      ...
                      <TextField label="Observación" value={editForm.observacion} onChange={(e) => setEditForm((p) => ({ ...p, observacion: e.target.value }))} />
                      <div className="sm:col-span-2">
                        <FileUpload
                          label="Documento"
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    {!isClosed && <Button variant="ghost" size="icon" aria-label={`Editar documento${item.tipo_documento ? `: ${item.tipo_documento}` : ""}`} onClick={() => startEdit(item)}><PencilIcon className="size-4" /></Button>}
                  </div>
                )}
              </CardContent>
            </Card>
